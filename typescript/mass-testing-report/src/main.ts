...
async function createReproducer(name: string): Promise<Uint8Array> {
...
// failures/crashes/timeouts/successes
function populateGeneralStats(element: HTMLElement, db: Database, meta: Metadata) {
    ...
    const stat_percent = (val: any) => percent(val, stats["total"]);
    const cpuCount = meta.cpu_count;
    const clockTime = (stats["total_time"] as number) / cpuCount;
    pre.innerHTML = `Statistics:
Total run: ${stats["total"]}
Successes: ${stats["ok"]} (${stat_percent(stats["ok"])})
Failures: ${stats["failed"]} (${stat_percent(stats["failed"])})
Crashes: ${stats["crashed"]} (${stat_percent(stats["crashed"])})
Timed out: ${stats["timed_out"]} (${stat_percent(stats["timed_out"])})

Start time: ${new Date(meta.start_time * 1000).toISOString()}
Total runtime: ${formatTime(clockTime, true)} (CPU count: ${cpuCount})`;
    ...
    const chart = new ApexCharts(pieDiv, {
        ...
        series: [stats["ok"], stats["failed"], stats["crashed"], stats["timed_out"]],
        labels: ["Successes", "Failures", "Crashes", "Timed out"],
        colors: ["#00e396", "#e6c000", "#f93636", "#fc923c"],
    });
    chart.render();
}
...
function getColumns(meta: Metadata): Columns[] {
    ...
    tableColumns.push({
        ...
        render: function (data, type, row) {
            ...
            container.appendChild(
                button("Repro", async () => {
                    const tar = await createReproducer(row.name);
                    saveAs(
...
function populateDatatable(element: Element, db: Database, meta: Metadata) {
    ...
    function searchBar(settings: { api: Api<any> }): HTMLElement {
        ...
        // page lags **a lot**
        sqlInput.addEventListener("input", () => updateState());
