...
def get_pipeline_description(pipeline: Pipeline) -> dict[str, Any]:
    ...
    sort_name = partial(sorted, key=lambda x: x["name"])
    ...
    pipeline_description = {
        ...
        "kinds": sort_name(get_singleton(Kind).type_dict()),  # type: ignore
        ...
        "artifacts": sort_name([artifact.to_dict() for artifact in pipeline.artifacts.values()]),
        "analyses": sort_name([analysis.to_dict() for analysis in pipeline.analyses.values()]),
        "analyses_lists": sort_name(
