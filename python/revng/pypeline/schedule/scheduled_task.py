...
class ScheduledTaskBase(ABC):
    ...
    ScheduledTaskBase objects (which are specialized for the task that will be
    ...
    def __init__(
        ...
        dependencies: list[ScheduledTaskBase] | None = None,
    ):
        self.node = node
        ...
        self.dependencies: list[ScheduledTaskBase] = []
        """
    ...
    def all_dependencies(self) -> Generator[ScheduledTaskBase, None, None]:
        """
...
class PipeScheduledTask(ScheduledTaskBase):
...
class SavepointScheduledTask(ScheduledTaskBase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert isinstance(self.node.task, SavePoint)

        self.responsible_pipes: list[PipeScheduledTask] = []
    ...
    def _run_impl(self, containers: ContainerSet, runner_context: RunnerContext):
        ...
        for pipe_task in self.responsible_pipes:
