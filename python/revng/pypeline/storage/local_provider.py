...
class LocalStorageProvider(StorageProvider):
    ...
    def _write_metadata(self, cursor: sqlite3.Cursor, type_: _MetadataUpdate):
        if type_ == _MetadataUpdate.FETCH:
            change_column = "last_fetch"
        elif type_ == _MetadataUpdate.OBJECT_SAVE:
            change_column = "last_object_save"
        elif type_ == _MetadataUpdate.MODEL_SAVE:
            change_column = "last_model_save"
        else:
            raise ValueError
    ...
    def set_model(
        ...
        invalidated = self._invalidate(invalidation_list, additional_objects)
