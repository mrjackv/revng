...
from revng.pypeline.utils import PypelineException, join_url
...
class _LockRenewalThread(threading.Thread):
    ...
    def add_lock(self, lock_id: str, headers: dict[str, str], refresh_interval: float):
        acutal_refresh_interval = refresh_interval * (1 - self.__class__.REFRESH_MARGIN)
        with self.entries_lock:
            self.entries[lock_id] = _LockEntry(
                headers=headers,
                refresh_interval=acutal_refresh_interval,
                next_renewal=time.monotonic() + acutal_refresh_interval,
            )
            self.event.set()

    def remove_lock(self, lock_id: str):
        with self.entries_lock:
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self.entries) > 0:
                global_next_renewal = min(e.next_renewal for e in self.entries.values())
                next_wait_seconds = max(global_next_renewal - time.time(), 1.0)
            else:
                next_wait_seconds = 1.0
        ...
        with self.entries_lock:
            ...
            for lock_id in locks_to_bump:
                ...
                next_renewal = now + self.entries[lock_id].refresh_interval * (
                    1 - self.__class__.REFRESH_MARGIN
                )
...
class RSSStorageProviderFactory(StorageProviderFactory):
    ...
    async def get(
        ...
        async with ClientSession(headers=headers) as session:
            # Run metadata check
            await self._metadata_check(session, pipeline_description)

            # Acquire the lock
            async with session.post(
                self._join_url("/lock"), json={"lock_type": lock_type_str}
            ) as resp:
            ...
            try:
                ...
                self._renewal_thread.remove_lock(lock_id)
                # Release the lock
                with suppress(ClientError):
                    async with session.post(
                        self._join_url("/release-lock"), headers={"X-RSS-Lock-ID": lock_id}
                    ) as resp:
                        resp.raise_for_status()

    async def _metadata_check(self, session: ClientSession, pipeline_description: bytes) -> None:
        pipeline_hash = hashlib.sha256(pipeline_description).hexdigest()
        body = {"version": version, "pipeline_description_hash": pipeline_hash}

        async with session.post(self._join_url("/metadata/check"), json=body) as resp:
            if resp.status == 412:
                async with session.put(
                    self._join_url("/metadata/pipeline-description"),
                    data=pipeline_description,
                ) as resp2:
                    resp2.raise_for_status()
                async with session.post(self._join_url("/metadata/check"), json=body) as resp3:
...
class RSSStorageProvider(StorageProvider):
    ...
    def set_model(
        self,
        invalidation_list: ModelPathSet,
        additional_objects: list[ObjectsToInvalidate],
        new_model: Model,
    ) -> SetModelResult:
        invalidation_json = {
            "invalidation_list": list(invalidation_list),
            "additional_objects": [
                {
                    "savepoint_range": {
