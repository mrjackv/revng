...
linkSize(const MetaAddress &Address,
  ...
  } else if (auto Iterator = Function.BasicBlocks.find(Address);
...
targetFooterSize(const yield::BasicBlock &BasicBlock,
  ...
  yield::Graph::Dimension PrefixSize = BasicBlock.CommentIndicator.size() + 1;
...
basicBlockSize(const yield::BasicBlock &BasicBlock,
  ...
  bool IsNextInstructionInDelayedSlot = false;
  for (const auto &Instruction : BasicBlock.Instructions) {
    appendSize(Result,
               instructionSize(Instruction,
                               Configuration,
                               BasicBlock.CommentIndicator.size(),
                               IsNextInstructionInDelayedSlot));
    IsNextInstructionInDelayedSlot = Instruction.HasDelayedSlot;
  }
...
void yield::cfg::calculateNodeSizes(Graph &Graph,
  ...
  for (auto *Node : Graph.nodes()) {
    ...
    if (Node->Address.isValid()) {
      // A normal node.
      if (auto Iterator = Function.BasicBlocks.find(Node->Address);
          Iterator != Function.BasicBlocks.end()) {
