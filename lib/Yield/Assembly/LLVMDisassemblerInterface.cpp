  ...
  for (size_t Position = 0; Position < Markup.size(); ++Position) {
    ...
    if (WhitespaceCount != 0) {
      Result.Tags.emplace_back(yield::TagType::Whitespace,
                               Result.Raw.size(),
                               Result.Raw.size() + WhitespaceCount);
      Result.Raw += Markup.substr(Position, WhitespaceCount);
    ...
    if (Markup[Position] == '<') {
      ...
      Result.Tags.emplace_back(TagType, Result.Raw.size());
      Position += 4;
      ...
      CurrentTag.ToPosition = Result.Raw.size();
      ...
      if (!OpenTagStack.empty()) {
        ...
        Result.Raw += Markup[Position];
        continue;
      }

      size_t MnemonicStart = Result.Raw.size();
      ...
      Result.Raw += Markup.substr(Mnemonic->FullPosition, Mnemonic->FullSize);
