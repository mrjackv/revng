...
static std::tuple<std::string, const model::TypeDefinition *, bool>
buildFieldEdgeLabel(const model::Type &Type,
  ...
  if (const auto *Array = llvm::dyn_cast<model::ArrayType>(&Type)) {
    ...
    return buildFieldEdgeLabel(*Array->ElementType(),
                               std::move(Current),
                               false);

  } else if (const auto *D = llvm::dyn_cast<model::DefinedType>(&Type)) {
    return { std::move(Current), &D->unwrap(), false };

  } else if (const auto *P = llvm::dyn_cast<model::PointerType>(&Type)) {
    if (!Current.empty())
      Current += ",\\n";
    Current += "Pointer (" + std::to_string(P->PointerSize()) + " bytes)";
    return buildFieldEdgeLabel(*P->PointeeType(), std::move(Current), true);

  } else if (const auto *P = llvm::dyn_cast<model::PrimitiveType>(&Type)) {
    return { std::move(Current), nullptr, false };
...
void TypeSystemPrinter::print(const model::TypeDefinition &T) {
  ...
  while (not ToVisit.empty()) {
    ...
    for (auto [Index, Field] : llvm::enumerate(Fields)) {
      auto [Label, DefinitionPtr, IsPointer] = buildFieldEdgeLabel(*Field);
      ...
      uint64_t SuccID;
      auto It = NodesMap.find(DefinitionPtr);
      if (It != NodesMap.end()) {
      ...
      ToVisit.push_back(DefinitionPtr);
    }
