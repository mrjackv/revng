...
def test_invalidation(model, storage_provider):
    ...
    storage_provider.set_model({"/two"}, [], storage_provider.get_model()[0])
    ...
    storage_provider.set_model({"/one"}, [], storage_provider.get_model()[0])
...
def test_storage_invalidation(storage_provider: StorageProvider):
    ...
    result = storage_provider.set_model({"/root"}, [], storage_provider.get_model()[0])
    ...
    result = storage_provider.set_model({"/function1"}, [], storage_provider.get_model()[0])
    assert result.invalidated_objects == {
        ContainerLocation(2, container_id, configuration_id): {function1},
